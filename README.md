...
Внутренний протокол агента и все аналитические поля ведутся на английском.
Это уменьшает служебный текст и обычно экономит токены, не сокращая обязательные
проверки source/sink, вызовов, ограничений и достижимости. Только готовое поле
`comment` в самом конце переводится на русский для вставки в Svacer; выдержки
из исходников и ссылки `файл:строка` не переводятся. Язык не заменяет оптимизацию
контекста: приложение по-прежнему выдаёт каждому агенту только его маркер и
точно относящиеся к нему трассы/исходники.
...
8. На вкладке **«Обзор»** нажмите **«Начать анализ»**. Один агент выполняет
   один маркер за раз; при двух агентах одновременно назначаются два, остальные
   ждут и переходят в работу по мере завершения. Число выбранных маркеров не
   сокращает глубину анализа каждого из них.
9. Проверьте карточки и готовый отчёт. **«Отправить в Svacer»** используйте
   только после собственной проверки: программа покажет сводку и запросит
   отдельное дословное подтверждение перед записью.
