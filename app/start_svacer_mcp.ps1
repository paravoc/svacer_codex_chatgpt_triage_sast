...
if (-not (Test-Path -LiteralPath $pythonPath)) {
    [Console]::Error.WriteLine("Python environment not found. Run setup_mcp.ps1 first.")
    exit 2
}

$settings = Get-Content -LiteralPath $settingsPath -Raw -Encoding UTF8 | ConvertFrom-Json
if ([string]::IsNullOrWhiteSpace([string]$settings.svacer_url)) {
    [Console]::Error.WriteLine("svacer_url is missing in svacer-settings.json")
    exit 2
}

# The dialog keeps Svacer credentials in memory only. Nothing is written to disk,
# Codex config, command history, or process arguments.
Add-Type -AssemblyName System.Windows.Forms
Add-Type -AssemblyName System.Drawing
[System.Windows.Forms.Application]::EnableVisualStyles()

$form = New-Object System.Windows.Forms.Form
$form.Text = "Подключение Codex к Svacer"
...
if ($dialogResult -ne [System.Windows.Forms.DialogResult]::OK -or
    ...
    [Console]::Error.WriteLine("Svacer login was cancelled.")
    $passwordBox.Clear()
    $form.Dispose()
    exit 2
}
...
$env:SVACER_TRANSPORT = "stdio"
$env:SVACER_TOOLS = "get_projects,get_snapshots,get_warnings,get_markers,get_project_stats,get_project_groups,get_advanced_file_preview,get_diff"
$env:PYTHONUTF8 = "1"
...
try {
    & $pythonPath (Join-Path $PSScriptRoot "start_svacer_stdio.py")
    exit $LASTEXITCODE
}
finally {
